...
```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/stavxyz/showcal@main/dist/showcal.min.css">
<script src="https://cdn.jsdelivr.net/gh/stavxyz/showcal@main/dist/showcal.min.js"></script>
...
Put URLs in your Google Calendar event descriptions — showcal does the rest:
...
Pass event data server-side. Accepts showcal schema **or raw Google Calendar API JSON** — showcal auto-detects the format. Your server can just proxy and cache the Google Calendar API response with zero transform.
...
Point at your own endpoint returning showcal schema or raw Google Calendar API JSON.

```js
ShowCal.init({ el: '#cal', fetchUrl: 'https://your-api.com/events' });
```
...
ShowCal.init({
  ...
  // --- Data (pick one) ---
  data: { /* showcal or Google Calendar API JSON */ },
  fetchUrl: 'https://...',
  ...
  storageKeyPrefix: 'showcal',          // for multiple instances
  ...
  knownPlatforms: [
    ...ShowCal.DEFAULTS.knownPlatforms,
...
| Attribute | Maps to |
|-----------|---------|
| `data-showcal` | Enables auto-init (required) |
...
showcal collects images from three sources:
...
showcal detects file URLs in event descriptions and surfaces them as downloadable attachments. Any URL ending in a recognized file extension — `.pdf`, `.doc`, `.docx`, `.xls`, `.xlsx`, `.csv`, `.ppt`, `.pptx`, `.zip`, `.txt` — is extracted from the description text and added to the event's `attachments` array.
...
Directives let you control showcal behavior directly from event descriptions using a hashtag syntax. This is useful when you don't have access to code — you can add platform links, images, and metadata tags right inside a Google Calendar event description.
...
Both `showcal` and `ogcal` prefixes are supported (case-insensitive). Directives are stripped from the rendered description.
...
```
#showcal:instagram:savebigbend     → "Follow @savebigbend on Instagram"
#showcal:zoom:123456789            → "Join Zoom" (links to zoom.us/j/123456789)
#showcal:discord:AbCdEf            → "Join Discord" (links to discord.gg/AbCdEf)
#showcal:eventbrite:12345          → "RSVP on Eventbrite"
```

All 18 built-in platforms are supported. Aliases: `twitter` → X, `meet` → Google Meet, `forms` → Google Forms, `maps` → Google Maps.

Directives and URLs are deduplicated: `#showcal:instagram:foo` and `https://instagram.com/foo` produce the same canonical ID, so only one button renders.

### Image directives

```
#showcal:image:https://example.com/flyer.png    → adds image to gallery
...
```
#showcal:featured    → pins event to top of its date group, adds star badge
#showcal:hidden      → hides event from all views (still accessible via direct link)
```
...
```
#showcal:tag:fundraiser        → scalar tag badge "fundraiser"
#showcal:tag:outdoor           → scalar tag badge "outdoor"
#showcal:cost:$25              → key-value badge "cost: $25"
#showcal:capacity:50           → key-value badge "capacity: 50"
#showcal:rsvp:https://form.com → rendered as a link button "Rsvp"
```
...
When events have tags (via `#showcal:tag:` directives or key-value directives), a filter bar appears above the view. Tags display as clickable pills ordered by frequency.
...
**Note:** showcal manages client-side OG meta tags (`og:title`, `og:description`, `og:image`) when viewing event details. This works for JavaScript-rendering crawlers (like Google) but social media crawlers (Facebook, Twitter) that don't execute JS will not see these tags. For full social sharing support, use server-side rendering or a prerender service.
...
showcal uses `Intl.DateTimeFormat` for all date/time formatting. Set `locale` and `weekStartDay` to match your audience. All UI strings are overridable via `i18n`.

## Multiple Instances

```js
ShowCal.init({ el: '#events-a', storageKeyPrefix: 'cal-a', ... });
ShowCal.init({ el: '#events-b', storageKeyPrefix: 'cal-b', ... });
```
...
- [esbuild](https://esbuild.github.io/) for bundling (IIFE format, `ShowCal` global)
- [marked](https://github.com/markedjs/marked) for markdown (bundled)
- CSS custom properties for theming
- `Intl.DateTimeFormat` for locale-aware formatting

Build outputs: `dist/showcal.js` (+ sourcemap), `dist/showcal.min.js`, `dist/showcal.css`, `dist/showcal.min.css`. The default config object is available as `ShowCal.DEFAULTS` for extending (e.g., `ShowCal.DEFAULTS.knownPlatforms`).
