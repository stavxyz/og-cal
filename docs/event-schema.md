...
| `featured` | `boolean` | `true` if `#showcal:featured` directive is present |
| `hidden` | `boolean` | `true` if `#showcal:hidden` directive is present |
...
1. **Directives** — `#showcal:` and `#showcal:` tokens are extracted and removed from the description. Platform directives become links, image directives become images, tag directives become tags, and `featured`/`hidden` flags are set.
...
Tokens are deduplicated — a directive and a URL pointing to the same resource produce one entry (e.g., `#showcal:instagram:foo` and `https://instagram.com/foo` are merged).
  ...
  events: [
    {
      id: 'unique-id',
      title: 'Event Title',
      description: 'Description text with #showcal:tag:outdoor directives...',
      location: 'Austin, TX',
...
showcal also accepts **raw Google Calendar API JSON** (the response from `calendars/{id}/events`). It auto-detects the format by checking for an `items` array and transforms it.
