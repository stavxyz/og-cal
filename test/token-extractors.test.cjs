...
describe("extractLinkTokens", () => {
  it("produces tokens with canonicalId for Instagram URL", () => {
    const result = extractLinkTokens(
      "Follow us https://www.instagram.com/savebigbend/",
    );
  ...
  it("deduplicates x.com and twitter.com URLs", () => {
    const result = extractLinkTokens(
      "https://x.com/foo https://twitter.com/foo",
    );
    assert.strictEqual(result.tokens.length, 1);
...
describe("extractImageTokens — Dropbox", () => {
  it("produces token with Dropbox canonical ID", () => {
    const result = extractImageTokens(
      "https://www.dropbox.com/scl/fi/abc123/photo.png?rlkey=xyz&dl=0",
    );
    assert.strictEqual(result.tokens.length, 1);
    assert.strictEqual(
      result.tokens[0].canonicalId,
...
describe("extractAttachmentTokens", () => {
  it("produces tokens with canonicalId for PDF URL", () => {
    ...
    assert.strictEqual(
      result.tokens[0].canonicalId,
      "attachment:example.com/report.pdf",
    );
    assert.strictEqual(result.tokens[0].source, "url");
