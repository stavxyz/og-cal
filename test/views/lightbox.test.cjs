...
describe("openLightbox", () => {
  ...
  it("navigates to next image on next button click", () => {
    ...
    document.querySelector(".already-lightbox-next").click();
    assert.strictEqual(
      document.querySelector(".already-lightbox-img").src,
      "https://a.com/2.jpg",
    );
    assert.strictEqual(
      document.querySelector(".already-lightbox-counter").textContent,
      "2 / 3",
    );
  });
  ...
  it("hides nav controls for single image", () => {
    ...
    assert.strictEqual(
      document.querySelector(".already-lightbox-counter"),
      null,
    );
  });
  ...
  it("traps Tab focus within the lightbox", () => {
    ...
    document.dispatchEvent(
      new window.KeyboardEvent("keydown", { key: "Tab", shiftKey: false }),
    );
    assert.strictEqual(document.activeElement, closeBtn);
  });
