...
describe("paginateEvents", () => {
  ...
  const futureEvents = Array.from({ length: 25 }, (_, i) =>
    createTestEvent({
      id: `f${i}`,
      title: `Future ${i}`,
      start: `2099-06-${String(i + 1).padStart(2, "0")}T10:00:00Z`,
    }),
  );
  ...
  const pastEvents = Array.from({ length: 15 }, (_, i) =>
    createTestEvent({
      id: `p${i}`,
      title: `Past ${i}`,
      start: `2020-01-${String(i + 1).padStart(2, "0")}T10:00:00Z`,
      end: `2020-01-${String(i + 1).padStart(2, "0")}T11:00:00Z`,
    }),
  );

  it("returns first pageSize events when showPast is false", () => {
    const result = paginateEvents(futureEvents, false, 10, {
      futureCount: 0,
      pastCount: 0,
    });
  ...
  it("returns no hasMoreFuture when all fit in one page", () => {
    ...
    const result = paginateEvents(small, false, 10, {
      futureCount: 0,
      pastCount: 0,
    });
  ...
  it("handles empty events", () => {
    const result = paginateEvents([], false, 10, {
      futureCount: 0,
      pastCount: 0,
    });
  ...
  it("handles null events", () => {
    const result = paginateEvents(null, false, 10, {
      futureCount: 0,
      pastCount: 0,
    });
    assert.strictEqual(result.visible.length, 0);
  ...
  it("splits past and future when showPast is true", () => {
    ...
    const result = paginateEvents(mixed, true, 10, {
      futureCount: 0,
      pastCount: 0,
    });
    assert.strictEqual(result.visible.length, 20);
  ...
  it("selects most recent past events first", () => {
    const mixed = [...pastEvents, ...futureEvents];
    const result = paginateEvents(mixed, true, 3, {
      futureCount: 0,
      pastCount: 0,
    });
  ...
  it("returns all future events when showPast is true but no past events exist", () => {
    const result = paginateEvents(futureEvents, true, 10, {
      futureCount: 0,
      pastCount: 0,
    });
    assert.strictEqual(result.visible.length, 10);
...
describe("renderPaginationButtons", () => {
  it("renders load-more button when hasMoreFuture is true", () => {
    ...
    renderPaginationButtons(
      ...
      paginated,
      {},
      { onLoadMore: () => {}, onShowEarlier: () => {} },
    );
    assert.strictEqual(
      bottom.querySelector(".already-load-more").textContent,
  ...
  it("renders show-earlier button when hasMorePast is true", () => {
    ...
    assert.strictEqual(
      top.querySelector(".already-show-earlier").textContent,
      "Show earlier (7 remaining)",
    );
  ...
  it("uses custom i18n labels", () => {
    ...
    renderPaginationButtons(top, bottom, paginated, i18n, {
      onLoadMore: () => {},
      onShowEarlier: () => {},
    });
    assert.strictEqual(
      bottom.querySelector(".already-load-more").textContent,
      "Más eventos (3 remaining)",
    );
    assert.strictEqual(
      top.querySelector(".already-show-earlier").textContent,
      "Eventos anteriores (2 remaining)",
    );
  });
