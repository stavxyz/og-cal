...
describe("createTagFilter", () => {
  ...
  it("renders tag pills for unique tags", () => {
    ...
    assert.strictEqual(
      container.querySelectorAll(".already-tag-pill").length,
      2,
    );
  });
  ...
  it('displays key-value text tags as "key: value"', () => {
    const { render } = createTagFilter(() => {});
    const container = document.createElement("div");
    const events = [createTestEvent({ tags: [{ key: "cost", value: "$25" }] })];
    render(container, events);
    assert.strictEqual(
      container.querySelector(".already-tag-pill").textContent,
      "cost: $25",
    );
  });
  ...
  it("uses i18n clearFilter label when provided", () => {
    ...
    container.querySelector(".already-tag-pill").click();
    assert.strictEqual(
      container.querySelector(".already-tag-clear").textContent,
      "Reset",
    );
  });
  ...
  it("union filter passes events matching any selected tag", () => {
    ...
    const pills = container.querySelectorAll(
      ".already-tag-pill:not(.already-tag-pill--active)",
    );
