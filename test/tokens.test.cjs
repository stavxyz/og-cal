...
describe("TokenSet", () => {
  it("accepts a token and retrieves it by type", () => {
    ...
    ts.add({
      canonicalId: "instagram:foo",
      type: "link",
      source: "url",
      url: "https://instagram.com/foo",
      label: "Follow @foo on Instagram",
      metadata: {},
    });
  ...
  it("rejects duplicate canonicalId and returns false", () => {
    ...
    assert.strictEqual(
      ts.add({ ...token, url: "https://www.instagram.com/foo/" }),
      false,
    );
  ...
  it("filters by type correctly across mixed types", () => {
    ...
    ts.add({
      canonicalId: "tag:outdoor",
      type: "tag",
      source: "directive",
      url: null,
      label: "outdoor",
      metadata: { key: "tag", value: "outdoor" },
    });
    assert.strictEqual(ts.ofType("link").length, 1);
...
describe("normalizeUrl", () => {
  it("strips www. prefix", () => {
    assert.strictEqual(
      normalizeUrl("https://www.instagram.com/foo"),
  ...
  it("normalizes http to https", () => {
    assert.strictEqual(
      normalizeUrl("http://instagram.com/foo"),
      "https://instagram.com/foo",
    );
  });
  ...
  it("removes trailing slash", () => {
    assert.strictEqual(
      normalizeUrl("https://instagram.com/foo/"),
      "https://instagram.com/foo",
    );
  });
  ...
  it("strips si param", () => {
    assert.strictEqual(
      normalizeUrl("https://example.com/page?si=abc"),
      "https://example.com/page",
    );
  });
  ...
  it("preserves non-tracking params", () => {
    assert.strictEqual(
      normalizeUrl("https://example.com/page?id=5&ref=home"),
      "https://example.com/page?id=5&ref=home",
    );
  });
  ...
  it("handles multiple tracking and non-tracking params", () => {
    assert.strictEqual(
      normalizeUrl(
        "https://example.com/page?id=5&utm_source=x&keep=1&fbclid=abc",
      ),
      "https://example.com/page?id=5&keep=1",
    );
  });
